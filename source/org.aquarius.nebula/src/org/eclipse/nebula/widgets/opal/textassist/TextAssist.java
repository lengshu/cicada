...
public class TextAssist extends Composite {
	...
	public TextAssist(final Composite parent, final int style, final TextAssistContentProvider contentProvider) {
		...
		this.popup = new Shell(getDisplay(), SWT.ON_TOP);
	...
	private void addTableListener() {
		this.table.addListener(SWT.Selection, event -> {
			...
			String sourceText = (String) this.table.getSelection()[0].getData(SOURCE_TEXT);
			this.text.setText(sourceText);
			this.popup.setVisible(false);
		});

		this.table.addListener(SWT.DefaultSelection, event -> {
			if (this.useSingleClick) {
				return;
			}
			String sourceText = (String) this.table.getSelection()[0].getData(SOURCE_TEXT);
		...
		this.table.addListener(SWT.KeyDown, event -> {
	...
	private Listener createKeyDownListener() {
		return new Listener() {
			...
			public void handleEvent(final Event event) {
				...
				if (event.keyCode == '/' && (event.stateMask == SWT.CTRL)) {
					...
					for (int i = 0; i < numberOfRows; i++) {
						...
						tableItem.setData(SOURCE_TEXT, stringValue);
					...
					final Point point = TextAssist.this.text.toDisplay(TextAssist.this.text.getLocation().x,
							TextAssist.this.text.getSize().y + TextAssist.this.text.getBorderWidth() - 3);
					int x = point.x;
					int y = point.y;
