...
/**
 * Rewrite this code to add change listener.<BR>
 *
 ...
 * {@link https://wiki.eclipse.org/Nebula_Multichoice }
 *
 * The MultiChoiceWithFixedWidth class represents a selectable user interface
 * object that combines a read-only text-field and a set of checkboxes.
 *
 * <p>
 ...
 */
public class MultiChoiceWithFixedWidth<T> extends Composite {
	...
	private Set<T> lastSelection = new TreeSet<T>();
	 ...
	 *                                     </ul>
	 */
	public MultiChoiceWithFixedWidth(final Composite parent, final int style) {
	 ...
	 *                                     </ul>
	 */
	public MultiChoiceWithFixedWidth(final Composite parent, final int style, final List<T> elements) {
		super(parent, style);
		...
		this.filter = new Listener() {
			...
			public void handleEvent(final Event event) {
				...
				if (shell == MultiChoiceWithFixedWidth.this.getShell()) {
	...
	private void createGlobalListener() {
		this.listener = new Listener() {
			@Override
			public void handleEvent(final Event event) {
				if (MultiChoiceWithFixedWidth.this.popup == event.widget) {
					handlePopupEvent(event);
					return;
				}

				if (MultiChoiceWithFixedWidth.this.arrow == event.widget) {
					handleButtonEvent(event);
					return;
				}

				if (MultiChoiceWithFixedWidth.this == event.widget) {
	...
	private void addListeners() {
		...
		if ((getStyle() & SWT.READ_ONLY) == 0) {
			final Listener validationListener = new Listener() {
				...
				@Override
				public void handleEvent(final Event event) {
					if (!MultiChoiceWithFixedWidth.this.popup.isDisposed() && !MultiChoiceWithFixedWidth.this.popup.isVisible()) {
						validateEntry();
					}
				}
			};
	...
	protected void validateEntry() {
		...
		getDisplay().asyncExec(() -> {
			...
			mb.open();
			MultiChoiceWithFixedWidth.this.text.forceFocus();
		});
	...
	public void setSelection(final Set<T> selection) {
		...
		this.selection = new TreeSet<T>(selection);
		this.lastSelection = new TreeSet<T>(selection);
		updateSelection();
	}
		...
		for (final T o : this.elements) {
			...
			checkBoxButton.addListener(SWT.Selection, e -> {
				if (checkBoxButton.getSelection()) {
					MultiChoiceWithFixedWidth.this.selection.add(o);
				} else {
					MultiChoiceWithFixedWidth.this.selection.remove(o);
				}
				MultiChoiceWithFixedWidth.this.lastModified = o;
				setLabel();
			});
	...
	private void closePopup() {
		...
		if (null != this.choiceChangeListener) {

			if (!CollectionUtils.isEqualCollection(this.lastSelection, this.selection)) {
				this.choiceChangeListener.changed();
			}
		}

		this.lastSelection = new TreeSet<T>(this.selection);
	...
	/**
	 * @return the choiceChangeListener
	 */
	...
	/**
	 * @param choiceChangeListener the choiceChangeListener to set
	 */
