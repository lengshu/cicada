...
1. Windowsで、インターネットオプションを開いてください。[詳細設定]タブウィンドウのセキュリティサブアイテムに[証明書アドレスと一致しません]があります。チェックを外してください。
...
#### Youtubeなどの一部のWebサイトでは、ブラウザで表示すると画像が表示されるのに、通常の画像がプロパティビューに表示されないのはなぜですか。

1. YoutubeはGoogleが開発したwebp形式の画像を使用しており、Javaはこの形式の画像を公式にサポートしていないため、ブラウザからのみ表示でき、ビューには表示できません。
2.この関数は後で追加することを検討します。
...
####マルチスレッドのダウンロードが設定されているのに、ダウンロードに使用されるスレッドは1つまたは2つだけなのはなぜですか？

ダウンロード時にファイルがセグメント化されます。ダウンロードセグメントのサイズがファイル全体の長さを超える場合は、1つのスレッドのみがダウンロードに使用されます。たとえば、ダウンロードセグメントのデフォルトサイズは200Mです。ダウンロードするファイルが100Mしかない場合、プログラムは1つのスレッドのみを有効にします。より多くのスレッドを使用する場合は、ダウンロードセグメントを50M以下に設定できますが、これはお勧めしません。
